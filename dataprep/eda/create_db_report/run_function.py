...
def parse_tables(
    ...
    for view_name, value_ in json_view_dict.items():
        ...
        view_obj.num_columns(value_["num_of_cols"])
    ...
    for t, current_columns in json_table_dict.items():
        ...
        for c in current_columns.keys():
            if c in [
                ...
                for current_index in current_columns["indices"]:
                    ...
                    if col_string := current_columns["indices"][current_index][
                        "Column_name"
                    ]:
...
def parse_constraints(current_database: Database, json_table_dict: Dict[str, Any]):
    ...
    """
    existing_tables = current_database.getTablesMap()
    for t, columns in json_table_dict.items():
        current_table = existing_tables[t]
...
def generate_db_report(sql_engine: Engine, analyze: bool = False):
    ...
    for table in current_database.getTables():
        table_file_name = f"{table.getName()}.html"
    ...
    return html_main_index_page.page_writer(
